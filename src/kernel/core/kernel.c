...
/* Kernel shell removed - init process is automan.sqr */
...
static int detect_boot_drive(void) {
    ...
    panic(
        ...
        " - ~~~SATAPI Is not fully fixed, please use SATA or ATAPI to boot.~~~ SATAPI Works now. It's a hardware problem 1000000000%%\n"
...
void shell_process_entry(void) {
    ...
    /* Kernel shell removed - AutoMan (PID 1) spawns services */
...
/* Stored at kernel_main entry — virtual address of the MB2 info struct.
 * Used by sqrm.c to populate sqrm_kernel_api_t::multiboot2_header. */
static uint64_t g_kernel_mb2_ptr = 0;

uint64_t kernel_get_mb2_ptr(void) {
    return g_kernel_mb2_ptr;
}
...
void kernel_main(uint64_t mb2_ptr)
{
    ...
    */
    /* Release the splash screen lock so userland output reaches the framebuffer. */
    bootscreen_overlay_set_enabled(0);
    VGA_Clear();
     ...
     * call below will switch to it.  When PID 1 later yields or blocks the
     * scheduler returns here, we hlt until the next timer tick, and repeat. */
    com_write_string(COM1_PORT, "[KERNEL] Entering idle loop...\n");
    __asm__ volatile("sti");
    for (;;) {
        schedule();
        __asm__ volatile("hlt");
