...
static void Interrupts_Init(void) {
    ...
    /* Interrupts are enabled after memory_system_init() so that any interrupt
     * handler (including fault paths) can safely call kmalloc. */
    COM_LOG_INFO(COM1_PORT, "IDT loaded, interrupts will be enabled after heap init");
}
...
static void init(uint64_t mb2_ptr_init) {
    ...
    /* Now that the heap is ready, it is safe to enable hardware interrupts. */
    ...
    /* GDT must be loaded first: writing a segment register in long mode zeros
     * the hidden base, which would clobber any MSR write done beforehand.
     * smp_init_bsp_early() sets GS_BASE after the GDT is stable. */
    amd64_gdt_init();

    /* Set up per-CPU GS base for BSP after the GDT is in place. */
    smp_init_bsp_early();
