...
void fault_print_last_user_pf(void) {
    ...
    com_write_string(COM1_PORT, (const char *)g_last_user_pf.pname);
...
void fault_handler_page_fault(uint64_t error_code, interrupt_frame_t *frame) {
    ...
    if ((frame->cs & 3) == 3) {
        ...
        // Not a COW fixable fault: continue to stack growth check below
    }
...
after_heap_demand: ;
     ...
     */
    {
        ...
        if (p && p->is_user) {
            ...
            if (addr < user_hi && p->user_stack_top && p->user_stack_low && p->user_stack_limit) {
                ...
                char h[32];
                com_write_string(COM1_PORT, "[STACK_CHK] PID=");
                char buf[12];
                itoa((int)p->pid, buf, 10);
                com_write_string(COM1_PORT, buf);
                com_write_string(COM1_PORT, " fault_addr=");
                format_hex64(addr, h);
                com_write_string(COM1_PORT, h);
                com_write_string(COM1_PORT, " page=");
                format_hex64(page, h);
                com_write_string(COM1_PORT, h);
                com_write_string(COM1_PORT, " low=");
                format_hex64(p->user_stack_low, h);
                com_write_string(COM1_PORT, h);
                com_write_string(COM1_PORT, " limit=");
                format_hex64(p->user_stack_limit, h);
                com_write_string(COM1_PORT, h);
                com_write_string(COM1_PORT, " top=");
                format_hex64(p->user_stack_top, h);
                com_write_string(COM1_PORT, h);
                com_write_string(COM1_PORT, "\n");

                /* Simplified stack growth: just check if the fault is within the valid
                 * stack region (between user_stack_limit and user_stack_low).
                 * We don't check proximity to RSP because user_rsp is not reliably
                 * updated during syscalls (it requires parsing the interrupt frame). */
                if (page < p->user_stack_low && page >= p->user_stack_limit) {
                    com_write_string(COM1_PORT, "[STACK_GROW] Attempting to grow stack\n");
                    if (page < p->user_stack_top) {
                        uint64_t pa = phys_alloc_frame();
                        if (pa) {
                            int rc = paging_map_page(page, pa, PFLAG_PRESENT | PFLAG_WRITABLE | PFLAG_USER);
                            com_printf(COM1_PORT, "[STACK_GROW] PID=%u page=0x%016llx pa=0x%016llx rc=%d\n",
                                       p->pid, page, pa, rc);
                            if (rc == 0) {
                            ...
                            com_printf(COM1_PORT, "[STACK_GROW] PID=%u page=0x%016llx alloc failed\n",
                                       p->pid, page);
                        }
                    }
                }
    ...
    com_write_string(COM1_PORT, "\n[FAULT] ===== PAGE FAULT =====\n");
    com_write_string(COM1_PORT, "[FAULT] Faulting address (CR2): 0x");
    char fault_h[32];
    format_hex64(faulting_address, fault_h);
    com_write_string(COM1_PORT, fault_h);
    com_write_string(COM1_PORT, "\n[FAULT] Faulting RIP: 0x");
    format_hex64(frame->rip, fault_h);
    com_write_string(COM1_PORT, fault_h);
    com_write_string(COM1_PORT, "\n[FAULT] Error code: 0x");
    format_hex64(error_code, fault_h);
    com_write_string(COM1_PORT, fault_h);
    com_write_string(COM1_PORT, "\n[FAULT] CR2=0x");
