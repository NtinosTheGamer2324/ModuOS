    ...
    while (*in) {
        ...
        if (in[0] == '/' && in[1] == '/') {
            if (in == line || is_space(in[-1])) {
                break; // line comment only if at start or after whitespace
            }
        }
        *out++ = *in++;
    }
...
static int g_autostart_boot_slot = -1;
static char *g_autostart_text = NULL;

void autostart_process_entry(void) {
    int boot_slot = g_autostart_boot_slot;
    char *text = g_autostart_text;
    
    if (!text) {
        com_write_string(COM1_PORT, "[AUTO] no autostart text\n");
        process_exit(0);
        return;
    }
    ...
    kfree(text);
    g_autostart_text = NULL;
    
    com_write_string(COM1_PORT, "[AUTO] autostart process complete\n");
    process_exit(0);
}
...
void kernel_run_autostart(int boot_slot) {
    ...
    // Store in globals for autostart process
    g_autostart_boot_slot = boot_slot;
    g_autostart_text = text;

    // Create autostart as a kernel process
    process_t *autostart_proc = process_create("autostart", autostart_process_entry, 5);
    if (!autostart_proc) {
        com_write_string(COM1_PORT, "[AUTO] Failed to create autostart process\n");
        kfree(text);
        g_autostart_text = NULL;
        return;
    }

    com_write_string(COM1_PORT, "[AUTO] autostart process created (PID ");
    char pidbuf[16];
    itoa(autostart_proc->pid, pidbuf, 10);
