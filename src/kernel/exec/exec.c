...
/* Kernel shell removed - exec spawns userland processes */
...
int exec_run(const char *args, int wait_for_exit) {
    ...
    const char *cwd = current ? current->cwd : "/";
    join_path(cwd, argv[0], exec_path);
    ...
    if (proc) { proc->page_table = proc_cr3; proc->cr3 = proc_cr3; }
    ...
    for (int i = 15; i >= 0; i--) {
        uint8_t nibble = (proc->context.rip >> (i * 4)) & 0xF;
        char hex = nibble < 10 ? '0' + nibble : 'a' + (nibble - 10);
        com_write_byte(COM1_PORT, hex);
    }
    com_write_string(COM1_PORT, "\n");
    ...
    for (int i = 15; i >= 0; i--) {
        uint8_t nibble = (proc->context.rsp >> (i * 4)) & 0xF;
        char hex = nibble < 10 ? '0' + nibble : 'a' + (nibble - 10);
        com_write_byte(COM1_PORT, hex);
    }
    com_write_string(COM1_PORT, "\n");
    ...
    /* Background exec: do NOT yield here. The caller's scheduler loop (kernel_main
     * idle or shell) will call schedule() at its own safe point, switching to the
     * newly enqueued process without stranding the caller in an unreturn-able yield. */
