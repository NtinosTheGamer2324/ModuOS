...
// It targets VirtualBox (Graphics Controller is VMSVGA) and VMware.
...
/* Hardware-accelerated triangle using VMSVGA FIFO scanline rendering.
 * Strategy: Use SVGA_CMD_RECT_FILL for each scanline of the triangle.
 * This leverages GPU hardware acceleration instead of CPU pixel writes.
 */
static int draw_triangle_hw(const framebuffer_t *fb,
    ...
    /* Use flat shading (color0) for simplicity */
    (void)color1;
    (void)color2;
    ...
    /* Rasterize triangle scanline-by-scanline using hardware RECT_FILL.
     * Compute left/right edges and fill each horizontal span.
     */
    for (int32_t y = y_start; y <= y_end; y++) {
        int32_t x_left, x_right;
        
        /* Compute intersection with triangle edges at scanline y */
        if (y < y1) {
            /* Top half: edges 0->1 and 0->2 */
            if (y1 != y0) {
        ...
        uint32_t span_width = (uint32_t)(x_right - x_left + 1);
        if (span_width == 0) continue;
        
        /* Send RECT_FILL command for this scanline to GPU */
        svga_fifo_rect_fill_t cmd;
        cmd.cmd = SVGA_CMD_RECT_FILL;
        cmd.color = color0;
        cmd.x = (uint32_t)x_left;
        cmd.y = (uint32_t)y;
        cmd.w = span_width;
        cmd.h = 1;
        fifo_write(&cmd, sizeof(cmd));
    }
    
    /* Send UPDATE command for the bounding box */
    ...
    uint32_t bbox_w = (uint32_t)(max_x - min_x + 1);
    uint32_t bbox_h = (uint32_t)(y_end - y_start + 1);
    
    svga_fifo_update_t upd;
    upd.cmd = SVGA_CMD_UPDATE;
    upd.x = (uint32_t)min_x;
...
static void vmsvga_flush(const framebuffer_t *fb, uint32_t x, uint32_t y, uint32_t w, uint32_t h) {
    ...
    /* PERFORMANCE FIX: Do NOT wait for FIFO on every flush!
     * The host processes commands asynchronously. Only sync on register writes
     * that require immediate completion (mode changes, etc).
     * This eliminates the lag that made VMSVGA slower than QXL.
     */
}
...
int sqrm_module_init(const sqrm_kernel_api_t *api) {
    ...
    // Check for 3D capability support
    uint32_t caps = svga_in(SVGA_REG_CAPABILITIES);
    int has_3d = (caps & 0x04) ? 1 : 0;  // SVGA_CAP_3D = bit 2
    if (has_3d) {
        com("[VMSVGA] Device supports 3D (SVGA_CAP_3D)\n");
    } else {
        com("[VMSVGA] Device does not support 3D\n");
    }

    // Set capability flags
    g_dev.caps = 0;
    if (g_fifo) {
        g_dev.caps |= SQRM_GPU_CAP_2D_ACCEL;  // Hardware 2D acceleration via FIFO
    }

    // 3D: hardware-accelerated triangle rasterizer via FIFO
    if (has_3d && g_fifo) {
        g_dev.draw_triangle = draw_triangle_hw;
        g_dev.caps |= SQRM_GPU_CAP_3D_TRIANGLES;
        com("[VMSVGA] 3D triangles enabled (HARDWARE-accelerated via FIFO)\n");
    } else {
        g_dev.draw_triangle = NULL;
        if (has_3d) {
            com("[VMSVGA] 3D capability detected but FIFO unavailable\n");
        }
    }
    ...
    if (rc == 0) {
        ...
        // Sync once after initial registration to ensure visibility
        svga_wait_for_fifo();
    } else { 
