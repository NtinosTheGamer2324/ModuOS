...
void qxl_flush(const framebuffer_t *fb, uint32_t x, uint32_t y, uint32_t w, uint32_t h) {
    (void)fb;
    if (!g_ram_hdr) return;
...
int qxl_enumerate_modes(gfx_mode_t *out_modes, uint32_t max_modes) {
...
*/
    ...
    // Currently, native implementations are effectively broken. So we use Software implementations and now QXL acts as a basic LinearFB

    sqrm_gpu_device_t gpu = {
        .fb                 = g_fb,
        .flush              = qxl_flush,
        /* CPU-side rendering into QXL primary surface memory.
         * True QXL command-ring acceleration can be added later.
         */
        .fill_rect32_native = sw_fill_rect32_native,
        .blit_rect32        = sw_blit_rect32,
        .blit_from_sg32     = NULL,
        /* Software cursor composited into the framebuffer */
        .cursor_set_argb32  = sw_cursor_set_argb32,
        .cursor_move        = sw_cursor_move,
        .cursor_show        = sw_cursor_show,
