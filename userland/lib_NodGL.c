...
int NodGL_GetScreenResolution(NodGL_Device device,
...
void NodGL_ReleaseResource(NodGL_Device device, NodGL_Resource resource) {
    ...
    if (res) { res->in_use = 0; res->mapped_addr = NULL; }
}
    ...
    uint32_t thickness)
{
    ...
    while (1) {
        if (y == span_y) {
            ...
            span_start_x = x; span_y = y; span_len = 1;
        }
        if (x == x1 && y == y1) break;
        int e2 = err * 2;
    ...
    uint32_t tint)
{
    ...
    return NodGL_DrawTexture(ctx, texture, 0, 0,
                             (int32_t)x, (int32_t)y,
                             (uint32_t)((float)res->width  * scale_x),
                             (uint32_t)((float)res->height * scale_y));
}
