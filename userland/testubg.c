...
static int register_test_node(const char *path) {
    userfs_user_node_t node;
    memset(&node, 0, sizeof(node));
    node.path = path;
    node.owner_id = "TESTMANAPP";
    node.perms = USERFS_PERM_READ_WRITE;
    return userfs_register(&node);
}

int md_main(long argc, char** argv) {
    register_test_node("test/test1");


    int startTime = time_ms(); 


    while (time_ms() - startTime < 2000) {
        yield();
    }
