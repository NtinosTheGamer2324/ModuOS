...
fnt_font_t *fnt_load_font(const void *data, size_t size) {
    ...
    for (uint32_t i = 0; i < font->header.glyph_count; i++) {
        ...
        font->glyphs[i].width        = read_u16_le(p); p += 2;
        font->glyphs[i].bitmap_width = read_u16_le(p); p += 2;
        font->glyphs[i].bitmap_height= read_u16_le(p); p += 2;

        /* Bitmap is stored row-padded: ceil(bitmap_width/8) bytes per row. */
        uint32_t bytes_per_row = ((uint32_t)font->glyphs[i].bitmap_width + 7u) / 8u;
        size_t bitmap_bytes = (size_t)bytes_per_row * font->glyphs[i].bitmap_height;
